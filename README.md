...
一个面向手机和桌面浏览器的 Codex Remote Web UI，通过本机 `codex app-server` 操作 Linux 服务器。当前版本为 `0.19.4`。
...
`v0.19.4` 增加主要消息的时间与来源说明、断线/服务重启后的任务核对、写请求结果确认、文件审批关联与可选 Web Push，并改善阅读位置、弹窗和无障碍交互。自动回归、原生任务恢复及独立账号安装/升级/回滚已有实际证据；具体设备身份、安全接管、厂商推送与实机验收仍按相关文档列出的范围继续跟踪。
...
公开仓库只包含清洗后的程序、文档和测试，不包含服务器任务、项目文件、登录凭据或个人 GitHub 凭据。需要可复现的固定版本时，将上面的克隆命令替换为 `git clone --branch v0.19.4 --depth 1 https://github.com/pgycz2024/codex-pwa.git`；也可以直接从 GitHub Releases 下载对应版本的 ZIP。
